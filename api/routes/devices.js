...
router.put("/updatedevice", checkAuth, async (req, res) => {
  try {
    ...
    const userId = req.body.client.id;
    const dId = req.body.client.dId;
    const data = req.body.client.data;


    if (data == null) {

      const datadevice = await Device.find({ userId: userId, dId: dId });

      const response = {
        status: "success",
        datadevice: datadevice[0].data
      };
      ...
      if (data == true) {
        await Device.updateOne({ userId: userId, dId: dId }, { data: "true" });
      }

      if (data == false) {
        await Device.updateOne({ userId: userId, dId: dId }, { data: "false" });
      }
...
router.post("/updatedevice", checkAuth, async (req, res) => {
  try {
    ...
    const device = await Device.find({ userId: userId, dId: dId });

    if (device.length == 1) {

      await Device.updateOne({ userId: userId, dId: dId }, { password: newpassword });
...
router.post("/clientsdevices", checkAuth, async (req, res) => {
  try {

    let id = req.body.client.id;

    const client = await Device.find({ userId: id });
...
router.post("/device", checkAuth, async (req, res) => {
  try {
    ...
    if (req.body.admin == true) {
      ...
      newDevice.lat = req.body.latitud;

      newDevice.long = req.body.longitud;

      newDevice.data = req.body.datadevice;
...
router.get("/listposition", checkAuth, async (req, res) => {
  try {
